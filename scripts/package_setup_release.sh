...
#
# Writes: dist/bfr-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix bfr \
  --exe-name Bomberman_Fantasy_Race__Recompiled \
  --display-name "Bomberman Fantasy Race  Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Bomberman Fantasy Race disc" \
